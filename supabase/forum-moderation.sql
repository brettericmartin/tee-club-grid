...
-- RLS Policies for forum_rate_limits (system only)
-- No user policies - managed by backend functions
...
-- Function to increment helpful post count
CREATE OR REPLACE FUNCTION increment_helpful_posts()
RETURNS TRIGGER AS $$
BEGIN
...
-- Trigger for helpful reactions
CREATE TRIGGER track_helpful_posts
  AFTER INSERT ON forum_reactions
  FOR EACH ROW
  EXECUTE FUNCTION increment_helpful_posts();